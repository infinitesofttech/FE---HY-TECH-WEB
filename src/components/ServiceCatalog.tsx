    ...
    <section id="services" className="py-20 bg-[#F5F7FA] relative">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="text-center max-w-3xl mx-auto mb-12 space-y-3">
          <div className="inline-flex items-center gap-1.5 px-4 py-1.5 rounded-full bg-[#EEEEFF] text-[#5751E1] text-xs font-extrabold uppercase tracking-wider border border-[#C5C5FF]">
            <Globe className="w-3.5 h-3.5 text-[#5751E1]" />
            <span>All Services Directory</span>
          </div>
          <h2 className="text-3xl sm:text-4xl font-black text-[#0F1322] tracking-tight">
        ...
        {/* Edunex Category Tabs */}
        <div className="flex items-center gap-2 overflow-x-auto pb-4 mb-10 scrollbar-none">
          <button
            onClick={() => setActiveCategory("all")}
            className={`px-5 py-2.5 rounded-full text-xs font-extrabold whitespace-nowrap transition-all shadow-sm ${
              activeCategory === "all"
                ? "bg-[#5751E1] text-white shadow-md shadow-[#5751E1]/30 scale-105"
                : "bg-white text-slate-700 hover:bg-[#EEEEFF] border border-slate-200"
            }`}
          >
            All Services
          </button>
              ...
              <button
                ...
                className={`flex items-center gap-2 px-5 py-2.5 rounded-full text-xs font-extrabold whitespace-nowrap transition-all shadow-sm ${
                  activeCategory === cat.id
                    ? "bg-[#0F1322] text-white shadow-md scale-105"
                    : "bg-white text-slate-700 hover:bg-[#EEEEFF] border border-slate-200"
                }`}
              >
                <CatIcon className="w-3.5 h-3.5 text-[#5751E1]" />
        ...
        {filteredCategories.length === 0 ? (
          <div className="text-center py-16 bg-white rounded-[32px] border border-slate-200 p-8 shadow-sm">
            <FileSearch className="w-12 h-12 text-[#5751E1] mx-auto mb-3" />
            <h3 className="text-lg font-bold text-[#0F1322]">
              No services found
            </h3>
              ...
              return (
                <div key={category.id} className="space-y-6">
                  ...
                  <div className="flex items-center gap-3 border-b border-slate-200 pb-3">
                    <div className="p-2.5 rounded-full bg-[#EEEEFF] border border-[#C5C5FF] text-[#5751E1]">
                      <CatIcon className="w-5 h-5" />
                    </div>
                    <h3 className="text-xl font-black text-[#0F1322]">
                      {category.titleEn}
                    </h3>
                  </div>

                  {/* Edunex Product Cards Grid */}
                    ...
                    {category.items.map((item, idx) => {
                      ...
                      return (
                        <motion.div
                          ...
                          className="group bg-white rounded-[32px] p-6 border border-slate-200 shadow-sm hover:shadow-edunex-hover transition-all duration-300 flex flex-col justify-between cursor-pointer relative overflow-hidden"
                        >
                          {/* Item Badge */}
                          {item.badgeEn && (
                            <span className="absolute top-5 right-5 text-[10px] font-extrabold bg-[#FFFBEB] text-[#D97706] px-3 py-1 rounded-full border border-[#FDE68A]">
                              {item.badgeEn}
                            </span>
                          )}
                          ...
                          <div className="space-y-4">
                            {/* Icon Container */}
                            <div className="w-13 h-13 rounded-2xl bg-[#EEEEFF] border border-[#C5C5FF] text-[#5751E1] flex items-center justify-center group-hover:bg-[#5751E1] group-hover:text-white transition-all duration-300">
                              <ItemIcon className="w-6 h-6" />
                            </div>

                            <div>
                              <h4 className="text-base font-extrabold text-[#0F1322] group-hover:text-[#5751E1] transition-colors">
                                {item.titleEn}
                              </h4>
      ...
      {/* Detail Modal */}
      {selectedService && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-[#0F1322]/75 backdrop-blur-sm animate-in fade-in duration-200">
          <div className="bg-white rounded-[32px] max-w-lg w-full p-6 sm:p-8 shadow-2xl relative border border-slate-200 max-h-[90vh] overflow-y-auto">
            ...
            <div className="space-y-6">
              <div className="flex items-center gap-3">
                <div className="p-3 bg-[#EEEEFF] text-[#5751E1] border border-[#C5C5FF] rounded-full">
                  {React.createElement(iconMap[selectedService.iconName] || Globe, { className: "w-7 h-7" })}
                </div>
                <div>
                  <h3 className="text-lg font-black text-[#0F1322]">
                    {selectedService.titleEn}
                  </h3>
                  <span className="text-xs text-[#5751E1] font-extrabold">HY-TECH Dharampur Service</span>
                </div>
              </div>
              ...
              {selectedService.requiredDocsEn && (
                <div className="space-y-2">
                  <h4 className="text-xs font-extrabold text-[#0F1322] uppercase tracking-wider flex items-center gap-1.5">
              ...
              <div className="space-y-2 pt-2">
                <a
                  ...
                  className="w-full py-4 bg-[#5751E1] hover:bg-[#4338CA] text-white font-extrabold rounded-full shadow-md text-xs flex items-center justify-center gap-2 transition-all"
                >
